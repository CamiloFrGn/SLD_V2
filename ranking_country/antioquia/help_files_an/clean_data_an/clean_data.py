...
def clean_data(get_clients_df,volumen_attribute_df,
               ...
               payment_discipline_attribute_df,fidelity_attribute_df,tools_adoption_attribute_df,cluster):
    try:
        print("here")
        #start merging
        #merge volume
        ...
        df['fidelity_value'] = df['fidelity_value'].fillna(0)
        ...
        df['tools_adoption_final'] = round(df['tools_adoption_final'],2)
        ...
        def conditions(df):
            if (df['% cumulative vol'] < 0.5):
                x = "PLATINUM"
            if (df['% cumulative vol'] >= 0.5) and (df['% cumulative vol'] < 0.66):
                x = "GOLD"
            if (df['% cumulative vol'] >=0.66):
                x = "SILVER"
