...
def clean_data(get_clients_df,volumen_attribute_df,sales_recurrence_attribute_df,
    ...
    try:
        ...
        #merge recurrence
        df = pd.merge(df,sales_recurrence_attribute_df,on="Cliente",how="left")
        #merge time in site
        ...
        df['total_recurrence'] = df['total_recurrence'].fillna(0)
        df['recurrence_final'] = df['recurrence_final'].fillna(0)
        ...
        df['growth_potential_final'] = df['growth_potential_final'].fillna(0)
        ...
        #create sld column
        def conditions(df):
            if (df['% cumulative vol'] < 0.5):
                x = "PLATINUM"
            if (df['% cumulative vol'] >= 0.5) and (df['% cumulative vol'] < 0.75):
                x = "GOLD"
