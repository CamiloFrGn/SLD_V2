...
def volumen_attribute(cluster,today_date_dt,relativedelta,query_sql_df):
    try:
        ...
        def conditions(df_volumen_attribute):
            if (df_volumen_attribute['volume'] <= 1000 ):
                x = (0.2*weight)*100
            if (df_volumen_attribute['volume'] > 1000 and df_volumen_attribute['volume'] <= 2000):
                x = (0.4*weight)*100
            if (df_volumen_attribute['volume'] > 2000 and df_volumen_attribute['volume'] <= 4000):
                x = (0.6*weight)*100
            if (df_volumen_attribute['volume'] > 4000 and df_volumen_attribute['volume'] <= 5000):
                x = (0.8*weight)*100
            if (df_volumen_attribute['volume'] > 5000 ):
                x = (weight)*100
