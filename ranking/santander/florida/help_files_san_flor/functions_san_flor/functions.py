...
def cancelations_attribute(plant,query_sql_df,get_clients_df):
    try:
      
        weight = 0.05
        sql_call_client_list = "{CALL sld_cancelations_plant (?)}"
        profits_list = query_sql_df(sql_call_client_list,(plant))
        profits_list = pd.merge(get_clients_df['Cliente'],profits_list,on="Cliente",how="left")
...
def fidelity_integrated_attribute(query_sql_df,get_clients_df):
    try:
     
        weight = 0.1
        sql_call_client_list = "{CALL sld_fidelity_integrated_sp ()}"
        fidelity_list = query_sql_df(sql_call_client_list,())
        fidelity_list = pd.merge(get_clients_df['Cliente'],fidelity_list,on="Cliente",how="left")

        fidelity_list['total_fidelity_integrated'] = fidelity_list['total_fidelity_integrated'].fillna(0) 
        fidelity_list['update_date_fidelity_integrated'] = fidelity_list['update_date_fidelity_integrated'].fillna(fidelity_list['update_date_fidelity_integrated'][0])

        fidelity_list['fidelity_integrated_final'] = fidelity_list['total_fidelity_integrated']*weight*(100/5)
...
def brand_permanence_attribute(plant,query_sql_df,get_clients_df):
    try:

        weight = 0.05
        sql_call_client_list = "{CALL sld_brand_permanence_attribute_sp (?)}"
        profits_list = query_sql_df(sql_call_client_list,(plant))
        profits_list = pd.merge(get_clients_df['Cliente'],profits_list,on="Cliente",how="left")
        profits_list['years_to_date'] = profits_list['years_to_date'].fillna(0)
        profits_list['years_to_date'] = profits_list['years_to_date'].astype(int)
        profits_list = profits_list.sort_values(by=["years_to_date"], ascending=[False])
        max_value = pd.DataFrame()
        max_value['years_to_date'] = profits_list['years_to_date']
        max_value = max_value.loc[max_value['years_to_date'].idxmax()]
        def conditions(profits_list):
            if (profits_list['years_to_date'] <= 0):
                x = 0
            if (profits_list['years_to_date'] > 0):
                x = (profits_list['years_to_date']/max_value)*weight*100
            return x
        
        profits_list['brand_permanence_final'] = profits_list.apply(conditions, axis=1)
...
def value_proposition_attribute(query_sql_df,get_clients_df):
    try:
     
        weight = 0.05
        sql_call_client_list = " execute sld_value_proposition_attribute_sp"
        fidelity_list = query_sql_df(sql_call_client_list,())
        fidelity_list = pd.merge(get_clients_df['Cliente'],fidelity_list,on="Cliente",how="left")
        fidelity_list['total_value_proposition'] = fidelity_list['total_value_proposition'].fillna(0)
        fidelity_list['update_date_value_proposition'] = fidelity_list['update_date_value_proposition'].fillna(fidelity_list['update_date_value_proposition'][0])
        
        
        def conditions(fidelity_list):
            if (fidelity_list['total_value_proposition'] > 0):
                x = (weight)*100
            else:
                x = 0
            return x
        
        fidelity_list['value_proposition_final'] = fidelity_list.apply(conditions, axis=1)
